 ...
 *
 * Portions derived from Microsoft's CLASSPNP sample.
 * Copyright (C) Microsoft Corporation, 1991 - 2010
 * QueueCache modifications: Copyright (c) 2019-2026 Olof Lagerkvist.
 *
...
typedef int INTERFACE_TYPE, KINTERRUPT_MODE, DMA_WIDTH, DMA_SPEED;
...
#define ASSERT( exp ) \
    ((!(exp)) ? \
        (assert( #exp, __FILE__, __LINE__, NULL ),FALSE) : \
        TRUE)
...
#define MAKE_CASE(scsiOpCode) case scsiOpCode: scsiOpStr = #scsiOpCode; break;
        ...
        MAKE_CASE(SCSIOP_TEST_UNIT_READY)
            ...
            MAKE_CASE(SCSIOP_GET_PERFORMANCE)
    }
